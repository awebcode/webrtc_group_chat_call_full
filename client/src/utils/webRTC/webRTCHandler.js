...
export const getLocalStream = () => {
  ...
  // Now use the polyfilled getUserMedia
  navigator.mediaDevices
    .getUserMedia(defaultConstrains)
    .then((stream) => {
      //at a  time in http or localhost not support multiple browsers
  ...
  if (!store.getState().call.screenSharingActive) {
    try {
      ...
      const sender = senders.find(
        (sender) => sender.track.kind === screenSharingStream.getVideoTracks()[0].kind
      );
